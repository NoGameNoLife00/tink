...
namespace tink {
    ...
    int DataPack::Pack(Message &msg, BytePtr &data, uint32_t &data_len) {
        return Pack(msg, data.get(), data_len);
    }


    int DataPack::Pack(Message &msg, byte *buff, uint32_t &data_len) {
    ...
    int DataPack::Unpack(BytePtr &data, Message &msg) {
        byte *ptr = data.get();
        // 读id
        uint32_t id = 0;
